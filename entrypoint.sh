...
# ==========================================================
    forge|neoforge)
      ...
      # --------------------------------------------
      # If run.sh exists → ALWAYS run it
      # --------------------------------------------
      if [[ -x "./run.sh" ]]; then
      ...
      # --------------------------------------------
      # First install ONLY (run.sh not yet generated)
      # --------------------------------------------
      log INFO "${TYPE} first-time install phase"

      # ★ installer を走らせるのはここだけ
      java @"${JVM_ARGS_FILE}" -jar "./server.jar" --installServer

      # run.sh が生成されていなければ失敗
      [[ -x "./run.sh" ]] || die "run.sh was not generated by ${TYPE} installer"

      log INFO "Install finished, re-entering runtime"
      exec "$0" run
      ;;
