...
lives in `scripts/lib/runtime_launch.sh`, and runtime launch dispatch has moved
there too. `generate_velocity_toml` now lives in `scripts/lib/velocity_config.sh`;
its existing call from Velocity artifact installation remains in place.
